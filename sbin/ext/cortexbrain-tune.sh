...
CPU_GOV_TWEAKS()
{
	...
	if [ "$cortexbrain_cpu" == "on" ]; then
		...
		local sampling_down_max_momentum_tmp="/sys/devices/system/cpu/cpufreq/$SYSTEM_GOVERNOR/sampling_down_max_momentum";
		if [ ! -e "$sampling_down_max_momentum_tmp" ]; then
			sampling_down_max_momentum_tmp="/dev/null";
		fi;

		local sampling_down_momentum_sensitivity_tmp="/sys/devices/system/cpu/cpufreq/$SYSTEM_GOVERNOR/sampling_down_momentum_sensitivity";
		if [ ! -e "$sampling_down_momentum_sensitivity_tmp" ]; then
			sampling_down_momentum_sensitivity_tmp="/dev/null";
		fi;

		local up_threshold_hotplug1_tmp="/sys/devices/system/cpu/cpufreq/$SYSTEM_GOVERNOR/up_threshold_hotplug1";
		if [ ! -e "$up_threshold_hotplug1_tmp" ]; then
			up_threshold_hotplug1_tmp="/dev/null";
		fi;

		local down_threshold_hotplug1_tmp="/sys/devices/system/cpu/cpufreq/$SYSTEM_GOVERNOR/down_threshold_hotplug1";
		if [ ! -e "$down_threshold_hotplug1_tmp" ]; then
			down_threshold_hotplug1_tmp="/dev/null";
		fi;

		local up_threshold_hotplug_freq1_tmp="/sys/devices/system/cpu/cpufreq/$SYSTEM_GOVERNOR/up_threshold_hotplug_freq1";
		if [ ! -e "$up_threshold_hotplug_freq1_tmp" ]; then
			up_threshold_hotplug_freq1_tmp="/dev/null";
		fi;

		local down_threshold_hotplug_freq1_tmp="/sys/devices/system/cpu/cpufreq/$SYSTEM_GOVERNOR/down_threshold_hotplug_freq1";
		if [ ! -e "$down_threshold_hotplug_freq1_tmp" ]; then
			down_threshold_hotplug_freq1_tmp="/dev/null";
		fi;
		...
		local auto_adjust_frequency_thresholds_tmp="/sys/devices/system/cpu/cpufreq/$SYSTEM_GOVERNOR/auto_adjust_frequency_thresholds";
		if [ ! -e "$auto_adjust_frequency_thresholds_tmp" ]; then
			auto_adjust_frequency_thresholds_tmp="/dev/null";
		fi;

		local afs_threshold1_tmp="/sys/devices/system/cpu/cpufreq/$SYSTEM_GOVERNOR/afs_threshold1";
		if [ ! -e "$afs_threshold1_tmp" ]; then
			afs_threshold1_tmp="/dev/null";
		fi;

		local afs_threshold2_tmp="/sys/devices/system/cpu/cpufreq/$SYSTEM_GOVERNOR/afs_threshold2";
		if [ ! -e "$afs_threshold2_tmp" ]; then
			afs_threshold2_tmp="/dev/null";
		fi;

		local afs_threshold3_tmp="/sys/devices/system/cpu/cpufreq/$SYSTEM_GOVERNOR/afs_threshold3";
		if [ ! -e "$afs_threshold3_tmp" ]; then
			afs_threshold3_tmp="/dev/null";
		fi;

		local afs_threshold4_tmp="/sys/devices/system/cpu/cpufreq/$SYSTEM_GOVERNOR/afs_threshold4";
		if [ ! -e "$afs_threshold4_tmp" ]; then
			afs_threshold4_tmp="/dev/null";
		fi;

		local profile_number_tmp="/sys/devices/system/cpu/cpufreq/$SYSTEM_GOVERNOR/profile_number";
